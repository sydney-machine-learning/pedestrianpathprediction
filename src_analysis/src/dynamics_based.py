...
class TransformerModel(nn.Module):
    ...
    def forward(self, src, mask):
        ...
        output, atts = self.transformer_encoder(src, mask=n_mask)
...
class DYNA(torch.nn.Module):
    ...
    def forward(self, inputs, iftest=False):
        ...
        # used for explainability, pending improvement
        atts_temporal_transformer1 = [] # get the last element since last element has all past frame
        atts_temporal_transformer2 = [] # get the last element since last element has all past frame
        atts_spatial_transformer1 = [] # get the avarage for dimension 1 (frame)
        atts_spatial_transformer2 = [] # get the avarage for dimension 1 (frame)
        ...
        for framenum in range(self.args.seq_length - 1):
            ...
            if framenum >= self.args.obs_length and iftest:
                ...
                nodes_current = outputs[self.args.obs_length - 1:framenum, node_index] - shift_value[self.args.obs_length:framenum + 1, node_index]
            ...
            spatial_input_embedded, atts_spatial_transformer1_ = self.spatial_encoder_1(spatial_input_embedded_[-1].unsqueeze(1), nei_list)
            # used for explainability, pending improvement
            atts_spatial_transformer1.append(atts_spatial_transformer1_)

            spatial_input_embedded = spatial_input_embedded.permute(1, 0, 2)[-1]

            temporal_input_embedded_last_, atts_temporal_transformer1_ = self.temporal_encoder_1(temporal_input_embedded)
            temporal_input_embedded_last = temporal_input_embedded_last_[-1]
            # used for explainability, pending improvement
            atts_temporal_transformer1.append(atts_temporal_transformer1_)
            ...
            spatial_input_embedded, atts_spatial_transformer2_ = self.spatial_encoder_2(fusion_feat.unsqueeze(1), nei_list)
            # used for explainability, pending improvement
            atts_spatial_transformer2.append(atts_spatial_transformer2_)
            ...
            temporal_input_embedded_, atts_temporal_transformer2_ = self.temporal_encoder_2(temporal_input_embedded)
            temporal_input_embedded = temporal_input_embedded_[-1]
            # used for explainability, pending improvement
            atts_temporal_transformer2.append(atts_temporal_transformer2_)
            ...
            # We use dynamical system with learned positive definite matrix to predict the next step
            if framenum >= self.args.obs_length and iftest:
                curr_pred_velocity, predicted_nextstep = self.get_predicted_velocity_and_nextstep(outputs_current, outputs[framenum-1, node_index], nodes_current[-1, node_index])
            else:
                curr_pred_velocity, predicted_nextstep = self.get_predicted_velocity_and_nextstep(outputs_current, nodes_abs[framenum, node_index], nodes_norm[framenum, node_index])
            outputs[framenum, node_index] = predicted_nextstep
    ...
    def get_predicted_velocity_and_nextstep(self, outputs_current, nodes_abs, nodes_norm):
        # This is dynamical system using learned positive definite matrix to predict the next step
        ped_num = outputs_current.shape[0] # pedestrian number
        sigma = 10 ** (-8)
        ...
        A_T = A.transpose(1,2)
        # This is the positive definite matrix
        pd_matrix = torch.bmm(A, A_T) + Sigma
        attractor_grad_func = lambda y: F.normalize(y)
        attractor_grad = -attractor_grad_func(nodes_norm)
        predicted_velocity = torch.bmm(pd_matrix, attractor_grad.unsqueeze(2)).squeeze() # This is the predicted velocity
        next_step = nodes_abs + 0.4 * predicted_velocity # This is the predicted next step
