...
class Trajectory_Dataloader():
    ...
    def traject_preprocess(self, setname):
        ...
        for seti, directory in enumerate(data_dirs):
            ...
            for ind, pedi in enumerate(Pedlist):
                ...
                if len(FrameList) < 2: # Filter the pedestrian trajectory with only one frame
                    continue
    ...
    def get_data_index(self, data_dict, setname, ifshuffle=True):
        ...
        if ifshuffle:
            r = random.random
            random.Random().shuffle(all_frame_id_list, random=r)
    ...
    def get_seq_from_index_balance(self, frameped_dict, pedtraject_dict, data_index, setname):
        ...
        for i in range(data_index.shape[1]):
            ...
            for ped in present_pedi:
                ...
                if iffull == False: # Just ignore the incomplete the trajectory
                    continue
                if len(cur_trajec) == 0:
                    continue
                if ifexistobs == False:
                    # if trajectory doesn't exsist at the last obversed time step
                    continue
    ...
    def massup_batch(self, batch_data):
        ...
        num_Ped_h = 0
        batch_pednum = []
        batch_velocity = np.zeros((self.args.seq_length, 0, 2)) # get the batch pedestrian walking velocity
        for batch in batch_data:
            num_Ped = batch.shape[1]
            seq_list, nei_list, nei_num, velocity = self.get_social_inputs_numpy(batch) # get the batch pedestrian walking velocity
            batch_velocity = np.append(batch_velocity, velocity, 1) # get the batch pedestrian walking velocity
            ...
            num_Ped_h += num_Ped
        return (nodes_batch_b, seq_list_b, nei_list_b, nei_num_b, batch_pednum, batch_velocity)
    ...
    def get_social_inputs_numpy(self, inputnodes):
        ...
        seq_length = inputnodes.shape[0]

        seq_list = np.zeros((inputnodes.shape[0], num_Peds))
        # denote where data not missing

        velocity = np.zeros(inputnodes.shape) # Get the pedestrian walking velocity
        ...
        return seq_list, nei_list, nei_num, velocity

    def rotate_shift_batch(self, batch_data, idx, ifrotate=True, iftest = False):
        '''
        Random ration and zero shifting.
        '''
        batch, seq_list, nei_list, nei_num, batch_pednum, batch_velocity = batch_data
        ...
        if ifrotate:
            ...
            # update velocity after rotating the batch data
        ...
        # We change the shift value from the last observed data to end point data for meeting the requirement of dynamical system
        if iftest: # During test, we use our predicted goal by goal predictor
            DATASET_NAME_TO_NUM = {
                'eth': 0,
                'hotel': 1,
                'zara1': 2,
            ...
            test_set_name = list(DATASET_NAME_TO_NUM.keys())[self.args.test_set]
            file_path = os.path.join(save_directory, f'goal_estimated_{test_set_name}.pkl')
            with open(file_path, 'rb') as f:
                predicted_file = pickle.load(f)
            s = predicted_file['Predicted_Goal'][idx]
        else:
            s = batch[self.args.seq_length - 1] # During train, we use ground truth goal

        shift_value = np.repeat(s.reshape((1, -1, 2)), self.args.seq_length, 0)

        batch_data = batch, batch - shift_value, shift_value, seq_list, nei_list, nei_num, batch_pednum, batch_velocity
        return batch_data

    def get_train_batch(self, idx):
        batch_data, batch_id = self.trainbatch[idx]
        batch_data = self.rotate_shift_batch(batch_data, idx, ifrotate=self.args.randomRotate, iftest = False)
        return batch_data, batch_id

    def get_test_batch(self, idx):
        batch_data, batch_id = self.testbatch[idx]
        batch_data = self.rotate_shift_batch(batch_data, idx, ifrotate=False, iftest = True)
        single_traj_type, if_interaction, interaction_type = get_type(batch_data[0], batch_data[6], self.args.test_set)
        traj_scene_type = single_traj_type, if_interaction, interaction_type
        return batch_data, batch_id, traj_scene_type
...
def L2forTestS(outputs, targets, obs_length, lossMask, num_samples=20):
    ...
    for index, value in enumerate(min_index):
        ...
        best_predicted_traj.append(outputs[value, :, index, :])

    best_predicted_traj = torch.stack(best_predicted_traj, dim=1)
    ...
    return error.item(), error_cnt, final_error.item(), final_error_cnt, best_predicted_traj, error_full_sum_min/12, best_error[-1]
